...
public class ConfigOld {
    ...
    public static void loadDefaults(FileConfiguration config) {
        ...
        if (MODE == null) {
            MODE = Mode.WHITELIST;
            ModObserver.LOGGER.warning("Found illegal property value, mode: " + mode + ", setting mode to \"whitelist\"");
        }
        try {
            ...
            ModObserver.LOGGER.warning("Can not load config defaults! Try deleting current config file to load them. If the issue persists, download the plugin again.");
        }

        saveValues(ModObserver.CONFIG);
    }
    ...
    public static List<String> getDefaultWhitelist(FileConfiguration configuration) {
        try {
            ...
            ModObserver.LOGGER.warning("Could not load default whitelist!");
            return null;
        }
    }
